...
*/
/**************************************************************************/
bool Adafruit_ADS1X15::begin(uint8_t i2c_addr /*= ADS1X15_ADDRESS*/, I2C_HandleTypeDef *hi2cx) {
    hi2c = hi2cx;
    return true;
}
...
void Adafruit_ADS1X15::writeRegister(uint8_t reg, uint16_t value) {
    ...
    buffer[1] = value >> 8;
    buffer[2] = value & 0xFF;
    HAL_I2C_Master_Transmit(hi2c, ADS1X15_ADDRESS << 1, buffer, 3, 0xff);
}
...
uint16_t Adafruit_ADS1X15::readRegister(uint8_t reg) {
    buffer[0] = reg;
    HAL_I2C_Master_Transmit(hi2c, ADS1X15_ADDRESS << 1, buffer, 1, 0xff);
    HAL_I2C_Master_Receive(hi2c, (ADS1X15_ADDRESS << 1) + 1, buffer, 2, 0xff);
